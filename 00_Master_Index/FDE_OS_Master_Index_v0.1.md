...
| ORG-003 | 公共能力维护方（Core） | 候选设计：资产索引维护+上浮评审合并权；**最小接口**=提交（技术交付负责人）→评审（维护方+原项目技术交付负责人）→批准/拒绝（维护方单方，理由登记）；泛化方法候选已建（DEL-014/TOOL-010/能力沉淀 v0.1）；现场有效性、具体人选、全局数值阈值与效果数据仍未知；**最小权限接口不变** | Designed（候选，仅最小接口） | DESIGN DECISION；依据=回流机制交集（3/4，形态各异） | EV-PAL-001/002（L1+L2）；EV-OAI-003（build-prove-generalize）；EV-LY-001④（4+X）；组织模型 v0.1.1 角色 C | Medium |
...
| DEL-008 | Baseline 阶段方法 | 方法草案已建（TOOL-004 v0.2）：先定**核心业务指标**（公式+统计单元），成本/时长/质量/量为**默认检查框架**按需取舍，先定口径后填数，单次记录不构成基线；**红队加边界 2026-08-18：拒测基线→不得作效果承诺，可转经批准标类的非效果任务或 Hold/退出；付费与否=商业决定**｜服务 GATE-2 与 R-C04 | Designed（草案，未现场验证） | DESIGN DECISION（方法采信） | EV-EXT-004 + EV-EXT-007（L1 锚点）；内部反例=结算协同单样本 | Medium |
...
| DEL-010 | 客户采用/ROI 阶段 | 已裁决（2026-08-18，DEC-2026-013/R-3，候选落位）：并入阶段 08 的 Exit 必查维度（实际使用率/业务结果对照基线/ROI 口径）+ GATE-7 证据输入；不设独立阶段；C 类备选（07a 采用确认）触发条件已登记；**红队加边界：绕过/申诉率入必查=观察信号，须先分因（质量/权限/摩擦/真抵触）再定动作** | Designed（候选落位，未现场验证） | DESIGN DECISION | 官网5；EV-LY-001 [18:11]；手册第 7 章 | Medium |
...
| TOOL-009 | 受控部署记录表（Controlled Deployment Sheet） | Draft V0.2（2026-08-18；四模式四要素+回滚十要素+漂移最小监测链） | `05_Field_Toolkit/TOOL-009_受控部署记录表_草案.md` |
| TOOL-010 | 上浮候选与泛化评审记录表（Edge→Core Review Sheet） | Draft V0.3.1（2026-08-18 最终验收 PASS；双入口+条件式分支+有条件准入语义唯一化；与 24 字段资产卡分工不复制） | `05_Field_Toolkit/TOOL-010_上浮候选与泛化评审记录表_草案.md` |

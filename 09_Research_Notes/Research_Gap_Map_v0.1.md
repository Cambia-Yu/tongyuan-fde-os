...
| G-06 | Deployment Leverage 与四健康指标 | **中（2026-08-17）**：Palantir 单组织实证（**毛利一手校正 FY2023：Palantir 81% vs Accenture 32.3%，Reviewer 核验，公司级背景不证 FDE 因果**、两指标）；跨组织仍缺 | High | P1（跨组织验证） | COM-004、H-4 |
| G-07 | Land & Expand vs Value Depth（事实/假设分界） | Low | Med-High | P1 | COM-002、H-5 |
| G-08 | Value Unit vs Pricing 各公司实证 | **中低（2026-08-18）**：瓴羊坐席/效果分成（单受访者）；Palantir 方向性（L2）；DeployCo/Tomoro 官方计费**未发现**（EV-OAI-004 负面检索）；行业级归纳仍缺 | | High | P1（跨组织验证） | COM-003、H-6 |
...
| G-15 | Deployment Unit Economics | **接口已建**（商业模型 §4 待采字段：收入确认/直接成本/支持成本/毛利/转转时间）；财务口径与数据全 Unknown | Med | 继续 open（不可被顺带关闭） | COM-004 |
